...
--[[
-- Function to check if any member from a list is present in the message
function isMemberInMessage(chat, members)
    if chat == nil or members == nil then
        return false
    end

    for _, member in ipairs(members) do
        if member ~= nil and type(member) == "string" then
            local pattern = "%W"..member:lower().."%W"
            if chat:lower():find(pattern) then
                return true
            end
        end
    end
...
-- Register the event to handle incoming text
windower.register_event('incoming text', function(chat, modified, chatMode, modifiedMode, blocked)
    -- Check if the incoming text contains either "effect wears off" or "no longer"
    -- also ensure it doesn't contain "Flash" or "Stun"
	-- also ensures it doesnt contain Impact debuffs when they where
    if chatMode >= 20 and
       not chat:lower():find("flash") and  -- Exclude messages containing "flash"
       not chat:lower():find("stun") and   -- Exclude messages containing "stun"
	   not chat:lower():find("STR") and   -- Exclude messages containing "STR"
	   not chat:lower():find("DEX") and   -- Exclude messages containing "DEX"
	   not chat:lower():find("INT") and   -- Exclude messages containing "INT"
	   not chat:lower():find("MND") and   -- Exclude messages containing "MND"
	   not chat:lower():find("VIT") and   -- Exclude messages containing "VIT"
	   not chat:lower():find("AGI") and   -- Exclude messages containing "AGI"
	   not chat:lower():find("CHR") and   -- Exclude messages containing "CHR"
       not (chat:lower():find(player.name:lower())) and  -- Check if your name is not in the message
       not isMemberInMessage(chat:lower(), party) and   -- Check if any party member's name is not in the message
       not isMemberInMessage(chat:lower(), alliance) then -- Check if any alliance member's name is not in the message

        if chat:find("effect wears off") or chat:find("no longer") then
            -- Convert chat to a string if it's not already
            chat = tostring(chat)
            
            -- Code to execute when the debuff effect wears off or is no longer active
            
            -- Send a party message with the chat text and "<call14>"
            send_command('@input /p ' .. chat .. ' <call14>')
