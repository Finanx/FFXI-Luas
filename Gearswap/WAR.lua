...
function user_setup()
	...
	state.WeaponSet = M{['description']='Weapon Set', 'Naegling', 'Loxotic_Mace', 'Shining_One', 'Chango', 'Ikenga_axe', 'Lycurgos', 'Dolichenus'}
    state.CP = M(false, "Capacity Points Mode")


	--Handles if the weapon from state.WeaponSet is 2 handed or not applies to engaged logic and dual wielded Weapons
	Two_Handed_Weapon_maps = S{"Shining_One", "Chango", "Lycurgos"}
	One_Handed_Weapon_maps = S{"Naegling", "Loxotic_Mace", "Ikenga_axe", "Dolichenus"}
...
function init_gear_sets()
    ...
    sets.engaged.Two_Handed.DT = sets.engaged.Hybrid
    sets.engaged.Two_Handed.Acc.DT = sets.engaged.Hybrid
    sets.engaged.Two_Handed.Defense = sets.engaged.Hybrid.Defense
    sets.engaged.Two_Handed.Acc.Defense = sets.engaged.Hybrid.Defense	
...
function update_combat_form()
	...
	if 	Two_Handed_Weapon_maps:contains(state.WeaponSet.value) then
	...
	if Two_Handed_Weapon_maps:contains(state.WeaponSet.value) then
		equip(sets[state.WeaponSet.value])
	elseif One_Handed_Weapon_maps:contains(state.WeaponSet.value) then
		if (player.sub_job == 'DNC' or player.sub_job == 'NIN') and sets[state.WeaponSet.value] and sets[state.WeaponSet.value].DW then
			equip(sets[state.WeaponSet.value].DW)
		else
			equip(sets[state.WeaponSet.value])
		end
	end
...
function update_combat_form()
	...
	if 	state.WeaponSet.value == 'Chango' or 
		state.WeaponSet.value == 'Lycurgos' or 
		state.WeaponSet.value == 'Shining_One' then
		 state.CombatForm:set('Two_Handed')
	else
		if DW == true then
			state.CombatForm:set('DW')
	...
	elseif state.WeaponSet.value == 'Ikenga_axe' then
		if player.sub_job == 'DNC' or player.sub_job == 'NIN' then
